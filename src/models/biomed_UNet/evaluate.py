...
from .scores import multiclass_dice_coeff, dice_coeff, multiclass_jaccard_coeff, calculate_pixelwise_classification_metrics, calculate_objectwise_classification_metrics, calculate_objectwise_classification_metrics2
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
            ...
            if net.n_classes == 1:
                ...
                dice_score += multiclass_jaccard_coeff(F.one_hot(mask_pred.argmax(dim=1), net.n_classes).permute(0, 3, 1, 2).float(), F.one_hot(mask_true, net.n_classes).permute(0, 3, 1, 2).float(), reduce_batch_first=False)
