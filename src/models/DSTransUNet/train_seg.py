...
from ..utils.losses import TreeSpotLocalizationLoss, PolygonCanopyLoss
...
def train_net(net,
    ...
    loss_function = TreeSpotLocalizationLoss(w_dice_weight=0.1, focal_tversky_weight=0.9, soft_l2_weight=0, tversky_alpha=0.2)
    loss_function = PolygonCanopyLoss(dice_weight=1.0, ssim_weight=0.0)
        ...
        with tqdm(total=n_train*len(size_rates), desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                for rate in size_rates:
                    ...
                    loss1 = loss_function(masks_pred, true_masks)#structure_loss_with_0_no_weigths(masks_pred, true_masks)
                    loss2 = loss_function(l2.squeeze(1), true_masks)#structure_loss_with_0_no_weigths(l2, true_masks)
                    loss3 = loss_function(l3.squeeze(1), true_masks)#structure_loss_with_0_no_weigths(l3, true_masks)
