...
class model(PreTrainedModel):
    def __init__(self, config):
        ...
        self.proj = nn.Linear(1024 // factor, self.d_model)  # Fix projection layer to match channels after down4
...
# 1. Added normalization to TransformerBlock with LayerNorm for better convergence.
# 2. Added dropout layers in DoubleConv and TransformerBlock to help reduce overfitting.
# 3. Modified the `Up` class to properly handle input channels after concatenation, ensuring consistency.
# 4. Added checkpointing to save memory for all encoder, decoder, and output layers.
# 5. Applied proper reshaping and permutation to ensure data flow consistency across transformer operations.
